...
public static class ToExtensions
{
    ...
    public static String ToAcronym(this String value, System.Globalization.CultureInfo culture, Boolean onlyPrincipalWords = true)
    {
        ...
        // ----------------------------------------------------------------------------------------------------
        onlyPrincipalWords = onlyPrincipalWords && Infrastructure.CultureInfoData.InitializeCultureData(culture);
        return Infrastructure.RegularExpressions.AlphaNumeric().Replace(value.ToAlphabetic(), new System.Text.RegularExpressions.MatchEvaluator(WordEvaluator)).RemoveWhiteSpace();
        String WordEvaluator(System.Text.RegularExpressions.Match word)
        {
            ...
            return String.IsNullOrWhiteSpace(word.Value)
                    || onlyPrincipalWords && (Infrastructure.CultureInfoData.InfoData.data?.Articles?.Contains(lower) ?? false)
                    || (Infrastructure.CultureInfoData.InfoData.data?.Conjunctions?.Contains(lower) ?? false)
                    || (Infrastructure.CultureInfoData.InfoData.data?.Prepositions?.Contains(lower) ?? false)
    ...
    public static String ToTitleCase(this String value, System.Globalization.CultureInfo culture)
    {
        ArgumentException.ThrowIfNullOrEmpty(value);
        ArgumentNullException.ThrowIfNull(culture);
        // ----------------------------------------------------------------------------------------------------
        if (!Infrastructure.CultureInfoData.InitializeCultureData(culture))
            return culture.TextInfo.ToTitleCase(value);
        else
        {
            String result = culture.TextInfo.ToTitleCase(value);
        ...
        String WordEvaluator(System.Text.RegularExpressions.Match word)
        {
            ...
            return word.Index > 0 
                   && !lastWord 
                   && word.Value.Length <= 3
                   && (Infrastructure.CultureInfoData.InfoData.data?.Articles?.Contains(lower) ?? false)
                   || (Infrastructure.CultureInfoData.InfoData.data?.Conjunctions?.Contains(lower) ?? false)
                   || (Infrastructure.CultureInfoData.InfoData.data?.Prepositions?.Contains(lower) ?? false)
                   ? lower 
                   : word.Value;
